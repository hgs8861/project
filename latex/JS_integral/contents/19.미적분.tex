...
수직선 위를 움직이는 점 P의 시각 $t$ 에서의 위치 $x$ 가 $x=f(t)$ 일 때,\\
 속도 $v(t)$ 는 $v(t) = \dfrac{dx}{dt} = f^{\prime}(t)$\\
$\int_{t_{0}}^{t} v(t) dt = f(t) - f(t_{0})$\\
시각 $t_{0}$ 에서의 점 P의 위치를 $x_{0}$ 이라고 하면 시각 $t$ 에서의 점 P의 위치 $f(t)$ 는\\
$f(t) = f(t_{0}) + \int_{t_{0}}^{t} v(t) dt = x_{0} + \int_{t_{0}}^{t} v(t) dt$\\
$t=a$ 일 때부터 $t=b$ 일 때까지 점 P의 위치의 변화량은\\
...
수직선 위를 움직이는 점 P의 시각 $t$ 에서의 속도가 $v(t)$ 일 때, $v(t)$ 가 연속이고 $v(t) \ge 0$ 이면 점 P가 움직인 거리는 
...
함수 $f(x)$ 구간 $[a, b]$ 에서 미분가능, 도함수 $f^{\prime}(x)$ 가 연속 \\
구간 $[a, b]$ 에서 곡선 $y=f(x)$ 의 길이 $l$ 은 
\[ l = \int_{a}^{b} \sqrt{1 + \{f^{\prime}(x)\}^2} dx \]
\end{conceptbox}

\begin{conceptbox}[기본 정석 - 평면 위의 운동]
좌표평면 위를 움직이는 점 P의 시각 $t$ 에서의 위치 $(x, y)$ 가 $x=f(t)$, $y=g(t)$, \\
구간 $[a, b]$ 에서 $f(t), g(t)$ 가 연속인 도함수 존재\\
이 구간에서 점 P가 움직인 거리 $l$ 은 
...
\begin{wrapfigure}[20]{r}{0.25\textwidth}
    \vspace{-30pt}
    \centering
    \includegraphics[width=0.25\textwidth]{graph39.png}
    \includegraphics[width=0.25\textwidth]{graph40.png}
    \includegraphics[width=0.25\textwidth]{graph41.png}
\end{wrapfigure}
구간 $[a, b]$ 를 $n$ 등분한 점과 양 끝 점을 $x_0(=a), x_1, x_2, \dots, x_n(=b)$, 이에 대응하는 곡선 위의 점을 $P_0, P_1, P_2, \dots, P_n$ 이라고 하자.\\
$ \overline{P_0P_1} + \overline{P_1P_2} + \overline{P_2P_3} + \dots + \overline{P_{n-1}P_n}$
은 $n \to \infty$ 일 때 곡선의 길이에 수렴 \\
평균값 정리에 의하여
$\dfrac{f(x_k) - f(x_{k-1})}{x_k - x_{k-1}} = f^{\prime}(t_k)$ \\
곧, $f(x_k) - f(x_{k-1}) = f^{\prime}(t_k)(x_k - x_{k-1})$
...
$n \to \infty$ 일 때 $t_k \to x_k$ 이므로 \\
이 합의 극한값은 $\int_{a}^{b} \sqrt{1 + \{f^{\prime}(x)\}^2} dx$ 이고, 이 값이 곡선의 길이이다.
\end{advicebox}
...
이것은 좌표평면 위를 움직이는 점 P가 $t=a$ 일 때부터 $t=b$ 일 때까지 움직인 거리와 같다.
\end{advicebox}
...
\begin{wrapfigure}[2]{l}{0.25\textwidth}
    \vspace{-30pt}
    \centering
    \includegraphics[width=0.25\textwidth]{graph42.png}
\end{wrapfigure} 
...
\begin{studybox}
  $t=a$ 일때부터 $t=b$ 일 때까지 점 P가 움직인 거리 $l$ 은 $\int_{a}^{b} \sqrt{(\dfrac{dx}{dt})^2 + (\dfrac{dy}{dt})^2} dt$ 
  따라서 속력은 $\sqrt{(\dfrac{dx}{dt})^2 + (\dfrac{dy}{dt})^2}$ 이다.
\end{studybox}

\textbf{모범답안} \\
(1) $\dfrac{dx}{dt} = -e^{-t}(\sin t + \cos t), \dfrac{dy}{dt} = -e^{-t}(\sin t - \cos t)$.\\
 속력은 $\sqrt{(\dfrac{dx}{dt})^2 + (\dfrac{dy}{dt})^2}= \sqrt{2}e^{-t}$ \\
