...
(1) $\sqrt{a^2-x^2}$ 꼴 : $x = a \sin \theta$ ($-\dfrac{\pi}{2} \le \theta \le \dfrac{\pi}{2}$)로 치환\\
(2) $a^2+x^2$ 꼴 : $x = a \tan \theta$ ($-\dfrac{\pi}{2} < \theta < \dfrac{\pi}{2}$)로 치환
\end{advicebox}
...
\begin{enumerate}[label=(\arabic*)]
    \item $\displaystyle\int_{1}^{4} \dfrac{(\sqrt{x}+1)^3}{\sqrt{x}} dx$ \\
    \item $\displaystyle\int_{-1}^{2} \dfrac{x}{x^2-x-6} dx$ \\
...
\textbf{모범답안}
\begin{enumerate}[label=(\arabic*)]
    \item $\sqrt{x}=t$로 놓으면 $\dfrac{1}{2\sqrt{x}}dx=dt$이다. $x=1 \to t=1, x=4 \to t=2$\\
    $\text{준식} = 2 \int_{1}^{2} (t+1)^3 dt = 2 \left[ \dfrac{1}{4}(t+1)^4 \right]_{1}^{2} = \dfrac{1}{2}(81-16) = \dfrac{65}{2}$
    \item $\dfrac{x}{(x-3)(x+2)} = \dfrac{1}{5} \left( \dfrac{3}{x-3} + \dfrac{2}{x+2} \right)$ 이므로\\
    $\text{준식} = \dfrac{1}{5} [3\ln|x-3| + 2\ln|x+2|]_{-1}^{2} = \dfrac{1}{5} \{(3\ln 1 + 2\ln 4) - (3\ln 4 + 2\ln 1)\} = -\dfrac{2}{5}\ln 2$
    \item $\int_{0}^{\pi} (e^{4x} - \dfrac{1-\cos 2x}{2}) dx = \left[ \dfrac{1}{4}e^{4x} - \dfrac{1}{2}x + \dfrac{1}{4}\sin 2x \right]_{0}^{\pi} = \dfrac{1}{4}(e^{4\pi}-1) - \dfrac{\pi}{2}$
\end{enumerate}
...
다음 정적분의 값을 구하여라.
\begin{enumerate}[label=(\arabic*)]
    \item $\displaystyle\int_{0}^{1} \dfrac{x^3}{x+1} dx + \int_{0}^{1} \dfrac{1}{t+1} dt$
...
\textbf{모범답안}
\begin{enumerate}[label=(\arabic*)]
    \item $\text{준식} = \int_{0}^{1} \dfrac{x^3+1}{x+1} dx = \int_{0}^{1} (x^2-x+1) dx = \left[ \dfrac{1}{3}x^3 - \dfrac{1}{2}x^2 + x \right]_{0}^{1} = \dfrac{5}{6}$
    \item $\text{준식} = \int_{0}^{\ln 3} \dfrac{e^{3x}}{e^x+1} dx + \int_{0}^{\ln 3} \dfrac{1}{e^x+1} dx = \int_{0}^{\ln 3} (e^{2x}-e^x+1) dx = \left[ \dfrac{1}{2}e^{2x}-e^x+x \right]_{0}^{\ln 3} = 2+\ln 3$
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item  
    \begin{wrapfigure}{l}{0.28\textwidth} % [h!] 옵션은 여기에(here!) 그림을 위치시키라는 의미
     \includegraphics[width=0.28\textwidth]{graph13.png} % 너비는 텍스트 너비의 70%    
    \end{wrapfigure}
    $e^x-2=0 \to x=\ln 2$. 구간 $[0, \ln 2]$에서 $e^x-2 \le 0$, $[\ln 2, 1]$에서 $e^x-2 \ge 0$\\
    $\text{준식} = \int_{0}^{\ln 2} (2-e^x) dx + \int_{\ln 2}^{1} (e^x-2) dx = [2x-e^x]_0^{\ln 2} + [e^x-2x]_{\ln 2}^1 = 4\ln 2 + e - 5$
    
    \item 
    \begin{wrapfigure}{l}{0.28\textwidth} % [h!] 옵션은 여기에(here!) 그림을 위치시키라는 의미
     \includegraphics[width=0.28\textwidth]{graph14.png} % 너비는 텍스트 너비의 70%    
    \end{wrapfigure}
    $\int_{0}^{\pi} \sin x dx + \int_{0}^{\pi} |\cos 2x| dx = [-\cos x]_0^\pi + 4\int_{0}^{\pi/4} \cos 2x dx = 2 + 4[\dfrac{1}{2}\sin 2x]_0^{\pi/4} = 4$
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item $\displaystyle\int_{0}^{1} \dfrac{e^x-1}{e^{2x}+e^{-x}} dx$
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    $\int_{1}^{-1} (1-t^3)t (-dt) = \int_{-1}^{1} (t-t^4) dt = 2\int_{0}^{1} (-t^4) dt = -\dfrac{2}{5}$
    \item $\sin x = t \to \cos x dx = dt$. $x: \pi/6 \to \pi/2 \Rightarrow t: 1/2 \to 1$\\
    $\int_{1/2}^{1} \dfrac{1}{t+t^3} dt = \int_{1/2}^{1} (\dfrac{1}{t} - \dfrac{t}{1+t^2}) dt = [\ln |t| - \dfrac{1}{2}\ln(1+t^2)]_{1/2}^1 = \dfrac{1}{2}\ln \dfrac{5}{2}$
    \item $\ln x = t \to \dfrac{1}{x}dx = dt$. $x: 1 \to e \Rightarrow t: 0 \to 1$\\
    $\int_{0}^{1} t dt = [\dfrac{1}{2}t^2]_0^1 = \dfrac{1}{2}$
    \item $e^x=t \to e^xdx=dt$. $\int_{1}^{e} \dfrac{t-1}{t^2+1/t} \cdot \dfrac{1}{t} dt = \int_{1}^{e} \dfrac{t-1}{t^3+1} dt = \int_{1}^{e} (\dfrac{-1}{t+1} + \dfrac{t}{t^2-t+1}) dt$\\
    계산하면 $\mathbf{\dfrac{1}{2}\ln \dfrac{e^2-e+1}{(e+1)^2} + \dfrac{\sqrt{3}\pi}{18}}$ (생략 가능)
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item $\displaystyle\int_{0}^{a} \dfrac{1}{x^2+a^2} dx$
\end{enumerate}

\begin{studybox}
삼각치환법을 이용한다.\\
(1) $x = a \sin \theta$\\
(2) $x = a \tan \theta$로 치환한다.
\end{studybox}

\textbf{모범답안}
\begin{enumerate}[label=(\arabic*)]
    \item $x=a\sin\theta, dx=a\cos\theta d\theta$. $x: 0 \to a \Rightarrow \theta: 0 \to \pi/2$\\
    $\int_{0}^{\pi/2} \sqrt{a^2-a^2\sin^2\theta} \cdot a\cos\theta d\theta = a^2 \int_{0}^{\pi/2} \cos^2\theta d\theta = a^2 \int_{0}^{\pi/2} \dfrac{1+\cos 2\theta}{2} d\theta = \dfrac{\pi a^2}{4}$
    \item $x=a\tan\theta, dx=a\sec^2\theta d\theta$. $x: 0 \to a \Rightarrow \theta: 0 \to \pi/4$\\
    $\int_{0}^{\pi/4} \dfrac{1}{a^2\sec^2\theta} \cdot a\sec^2\theta d\theta = \dfrac{1}{a} \int_{0}^{\pi/4} 1 d\theta = \dfrac{\pi}{4a}$
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    기함수는 $f(-x)=-f(x)$이므로 $\int_{0}^{a} -f(x) dx + \int_{0}^{a} f(x) dx =0$
\end{enumerate}
...
\textbf{모범답안}
\begin{enumerate}[label=(\arabic*)]
    \item $\int_{0}^{\pi/2} x\cos x dx - \int_{\pi/2}^{\pi} x\cos x dx = [x\sin x + \cos x]_0^{\pi/2} - [x\sin x + \cos x]_{\pi/2}^\pi = (\dfrac{\pi}{2}-1) - (-1-\dfrac{\pi}{2}) = \pi$
    \item $u = \ln(\sqrt{x^2+1}-x), v' = 1$이라 하면 $u' = \dfrac{-1}{\sqrt{x^2+1}}, v = x$\\
    $[x\ln(\sqrt{x^2+1}-x)]_0^1 + \int_0^1 \dfrac{x}{\sqrt{x^2+1}} dx = \ln(\sqrt{2}-1) + [\sqrt{x^2+1}]_0^1 = \ln(\sqrt{2}-1) + \sqrt{2}-1$
    \item $[x^2 \cdot \dfrac{1}{2}e^{2x}]_0^1 - \int_0^1 x e^{2x} dx = \dfrac{e^2}{2} - ([x \cdot \dfrac{1}{2}e^{2x}]_0^1 - \int_0^1 \dfrac{1}{2}e^{2x} dx) = \dfrac{e^2}{2} - \dfrac{e^2}{2} + [\dfrac{1}{4}e^{2x}]_0^1 = \dfrac{e^2-1}{4}$
\end{enumerate}
...
$a = \int_0^1 t(e^{-t}+bt) dt = [-te^{-t}-e^{-t}+\dfrac{b}{3}t^3]_0^1 = 1-\dfrac{2}{e}+\dfrac{b}{3}$\\
$b = \int_0^1 (t^2+a) dt = [\dfrac{1}{3}t^3+at]_0^1 = \dfrac{1}{3}+a$\\
연립하면 $a = \dfrac{5}{3}-\dfrac{3}{e}, b = 2-\dfrac{3}{e}$\\
$\therefore f(x) = x^2 + \dfrac{5}{3}-\dfrac{3}{e}, \quad g(x) = e^{-x} + (2-\dfrac{3}{e})x$
...
\begin{studybox}
(1) 부분적분을 두 번 사용하여 정적분을 구한다. \\
(2)는 지수함수의 극한 성질을 이용한다.
\end{studybox}

\textbf{모범답안}
\begin{enumerate}[label=(\arabic*)]
    \item $\int e^{ax}\cos x dx = \dfrac{e^{ax}(a\cos x + \sin x)}{a^2+1} + C$ 이므로\\
    $f(y) = \dfrac{e^{ay}(a\cos y + \sin y) - a}{a^2+1}$
    \item $y \to \infty$ 일 때 $e^{ay}$가 수렴해야 하므로 $a < 0$이다. 이때 극한값은 $-\dfrac{a}{a^2+1}$
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    \item $I_n = \dfrac{n-1}{n} I_{n-2}$ ($n \ge 2$)가 성립함을 증명하여라.
...
\begin{enumerate}[label=(\arabic*)]
    ...
    정리하면 $n I_n = (n-1)I_{n-2} \to I_n = \dfrac{n-1}{n} I_{n-2}$
    \item $I_5 = \dfrac{4}{5} \cdot \dfrac{2}{3} \cdot I_1 = \dfrac{8}{15} \cdot [-\cos x]_0^{\pi/2} = \dfrac{8}{15}$\\
    $I_{10} = \dfrac{9}{10} \cdot \dfrac{7}{8} \cdot \dfrac{5}{6} \cdot \dfrac{3}{4} \cdot \dfrac{1}{2} \cdot I_0 = \dfrac{63}{512} \cdot \dfrac{\pi}{2} = \dfrac{63\pi}{1024}s$
\end{enumerate}
