...
\begin{wrapfigure}{l}{0.25\textwidth}
    \centering
    \includegraphics[width=0.25\textwidth]{graph11.png}
\end{wrapfigure}
...
\begin{figure}[htbp] % [h!] 옵션은 여기에(here!) 그림을 위치시키라는 의미
  ...
  \includegraphics[width=0.2\textwidth]{graph15.png} % 너비는 텍스트 너비의 70%    
  \includegraphics[width=0.2\textwidth]{graph16.png} % 너비는 텍스트 너비의 70%    
\end{figure}

$S_{n}=\dfrac{1}{n^4}{0^3+1^3+\cdots+(n-1)^3}$, $T_{n}=\dfrac{1}{n^4}{1^3+2^3+\cdots+n^3}$ 이므로\\
$S_n < S < T_n$ 이고, $\displaystyle \lim_{n \to \infty} S_n = \displaystyle \lim_{n \to \infty} T_n$ 이므로 
$\therefore S = \displaystyle \lim_{n \to \infty} T_n = \displaystyle \lim_{n \to \infty} S_n=\dfrac{1}{4}$ 
...
\begin{wrapfigure}{r}{0.28\textwidth} % [h!] 옵션은 여기에(here!) 그림을 위치시키라는 의미
  \includegraphics[width=0.28\textwidth]{graph17.png} % 너비는 텍스트 너비의 70%    
\end{wrapfigure}
원뿔을 밑면에 평행한 평면으로 $n$ 등분하여 $n-1$ 개의 원기둥을 만들면 부피의 합 $V_n$ 은 \\
$V_{n}=\pi\times\dfrac{r^{2}h}{n^{3}}\{1^{2}+2^{2}+\cdot\cdot\cdot+(n-1)^{2}\} = \dfrac{\pi r^{2}h}{n^{3}}\times\dfrac{(n-1)n(2n-1)}{6}$ \\
$\therefore V = \displaystyle \lim_{n \to \infty} V_n = \dfrac{1}{3} \pi r^2 h$ 
...
\begin{studybox}
$\displaystyle \lim_{n \to \infty} \sum_{k=1}^{n} f\left(a + \dfrac{p}{n}k\right) \dfrac{p}{n}=\displaystyle \int_{a}^{a+p} f(x) dx=\displaystyle \int_{0}^{p} f(a+x) dx=\displaystyle p \int_{0}^{1} f(a+px) dx$
\end{studybox}

\textbf{모범답안} \\
\begin{enumerate}[label=(\arabic*)]
    \item $\displaystyle \dfrac{\sum_{k=1}^{n} (n+k)^3 \frac{1}{n^4}}{\sum_{k=1}^{n} k^3 \frac{1}{n^4}} = \dfrac{\sum_{k=1}^{n}(1+k/n)^3\frac{1}{n}}{\sum_{k=1}{n}(k/n)^3 \frac{1}{n}}=\dfrac{\int_0^1 (1+x)^3 dx}{\int_0^1 x^3 dx} = 15$
    \item $\displaystyle \lim_{n \to \infty} \dfrac{1}{n^3} \sum_{k=1}^{n} k^2 e^{k/n}$$\displaystyle \int_0^1 x \cos(\dfrac{\pi}{2}x^2) dx = \dfrac{1}{\pi}$
    \item $\displaystyle \int_0^1 x^2 e^x dx = e-2$ 
\end{enumerate}
...
\textbf{모범답안} \\
\begin{enumerate}[label=(\arabic*)]
    \item $\displaystyle \sum_{k=1}^{n} \dfrac{\pi}{n^2} k \cos \left( \dfrac{k\pi}{n} \right) = \pi \int_0^1 x \cos(\pi x) dx = -\dfrac{2}{\pi}$
    \item $\displaystyle \sum_{k=1}^{n} \ln \sqrt[n]{\dfrac{n+k}{n}} = \int_0^1 \ln(1+x) dx = 2\ln 2 - 1$ 
\end{enumerate}
...
\textbf{모범답안} \\
\begin{enumerate}[label=(\arabic*)]
    \item $\displaystyle \int_0^1 x \sqrt{1-x^2} dx = \dfrac{1}{3}$
    \item $\displaystyle \int_0^1 \dfrac{1+2x}{1+x^2} dx = \dfrac{\pi}{4} + \ln 2$
\end{enumerate}
...
\begin{figure}[htbp] % [h!] 옵션은 여기에(here!) 그림을 위치시키라는 의미
  \centering % 이미지를 가운데로 정렬
  \includegraphics[width=0.6\textwidth]{graph12.png} % 너비는 텍스트 너비의 70%    
\end{figure}
그림 1 에서 $1+\dfrac{1}{2}+\dfrac{1}{3}+\cdots+\dfrac{1}{n} > \int_{1}^{n+1} \dfrac{1}{x} dx$\\
그림 2 에서 $\dfrac{1}{2}+\dfrac{1}{3}+\cdots+\dfrac{1}{n} < \int_{1}^{n} \dfrac{1}{x} dx$ \\
$y=1/x$ 의 그래프에서 $S = 1 + \dots + 1/n$ 이라 하면, 영역의 넓이 비교에 의해 $S > \int_1^{n+1} \dfrac{1}{x} dx = \ln(n+1)$ 이고, $S-1 < \int_1^n \dfrac{1}{x} dx = \ln n$ 이므로 성립한다. 
...
\begin{studybox}
 $\displaystyle \lim_{x \to a} \dfrac{1}{x-a} \int_{a}^{x} f(t) dt = \lim_{x \to a} \dfrac{F(x)-F(a)}{x-a}=F'(a) = f(a)$ 
\end{studybox}

\textbf{모범답안} \\
\begin{enumerate}[label=(\arabic*)]
    \item $F^{\prime}(1) = 6$
    \item $\displaystyle \lim_{x \to 1} \dfrac{F(x^2)-F(1)}{x^3-1}$, $F^{\prime}(1) = e$ 이고, $\lim_{x \to 1} \dfrac{F(x^2)-F(1)}{x^{2}-1} \times \dfrac{x^2-1}{x^3-1}$ 
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item $\displaystyle \lim_{t \to \infty} t \int_{0}^{\dfrac{2}{t}} \dfrac{|x-1|}{x^2+2} dx$
\end{enumerate}

\begin{studybox}
 $\displaystyle \lim_{h \to 0}\dfrac{F(a+h)-F(a)}{h}=F'(a)=f(a) $ 
\end{studybox}

\textbf{모범답안} \\
\begin{enumerate} [label=(\arabic*)]
    \item $2F^{\prime}(1) = -2$
    \item  $\displaystyle \lim_{h \to 0+} \dfrac{1}{h} \int_0^{2h} f(x) dx = 2f(0) = 1$ 
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    \item $a=4, f(x)=2e^x-4$
    \item 양변을 미분하면 $f(\ln(x))\frac{1}{x}= 2x-1$ 이고\\
     이때 $\ln(x)=y$라고 하면 $f(y)=2e^(2y)-e^y$이다. $a=0$
\end{enumerate}
...
$-\dfrac{\pi}{2} < x < \dfrac{\pi}{2}$ 에서 정의된 함수 $f(x)$ 에 대하여 $f^{\prime}(x)$ 가 연속함수이고 $f(x) = \tan x - x - \displaystyle \int_{0}^{x} f^{\prime}(u) \tan^2 u du$ 일 때, $f^{\prime}(x)$ 와 $f(x)$ 를 구하여라. 

\textbf{모범답안} \\
$f'$이 연속이므로 $\int f'$은 미분 가능하다. 양변을 미분하면 $f^{\prime}(x) = \sec^2 x - 1 - f^{\prime}(x) \tan^2 x \implies f^{\prime}(x) = \sin^2 x$, 이를 적분하고 $f(0)=0$ 을 대입하면 $f(x) =\dfrac{1}{2}x - \dfrac{1}{4}\sin 2x$ 
...
\begin{studybox}
피적분 함수에 적분변수 t와 다른 변수 x가 포함되어 있으므로 
\[\int_{0}^{x} (x-t)f^{\prime}(t) dt = x \int_0^x f^{\prime}(t) dt - \int_0^x t f^{\prime}(t) dt \]
와 같이 변형하여 이용한다.
\end{studybox}

\textbf{모범답안} \\
양변을 두 번 미분하여 정리하면 \\
$f'(x)=\int_{0}^{x}f^{\prime}(t)dt+e^x+1=e^x(ax+a+b)$\\
$f^{\prime \prime}= f^{\prime}(x)+e^x=e^x(ax+2a+b)$ 이므로\\
 $e^x(ax+a+b+1)=e^x(ax+2a+b)$ 따라서 $a=1$, $f(0)=1=b$ 이므로 $a=1$, $b=1$ 
...
$\dfrac{d}{dx}\ \displaystyle \int_{a}^{x}f(t)dt=f(x)$ 임을 활용한다. 
\end{studybox}

\textbf{모범답안} \\
\begin{enumerate} [label=(\arabic*)]
    \item 극댓값 $f(-\pi)=f(\pi)=2$,극솟값 $f(0)=0$ 
    \item $f'(x)=e^{(x+1)^3-7(x+1)} - e^{x^3-7x}$ 이므로 지수만 비교 하여 극값을 찾으면 \\
    $(x+1)^3-7(x+1) = x^3-7x$에서 $x=-2, 1$일때 극값을 갖고 \\
    증감표를 확인하면 극대는 $f'(x)=0$ 에서 $x=-2$ 
\end{enumerate}
...
\begin{studybox}
피적분 함수 $f(x)$의 부정적분을 구하는 것은 쉽지 않으므로 다음과 같은 적당한 연속함수 $g(x), h(x)$ 를 찾는다.\\
적어도 한 실수 $x$ 에서 $f(x)=g(x)$ 또는 $f(x)=h(x)$를 만족하지 않을때,\\
 $g(x) \le f(x) \le h(x)$ 이면 $\displaystyle \int_{a}^{b} g(x) dx \le \int_{a}^{b} f(x) dx \le \int_{a}^{b} h(x) dx$ 
\end{studybox}

\textbf{모범답안} \\
\begin{enumerate} [label=(\arabic*)]
    \item $0 \le x \le 1$ 에서 $\dfrac{1}{2} \le \dfrac{1}{1+x^2} \le 1$ 이므로 \\
    $\int_{0}^{1}\dfrac{1}{2} x^n dx < \displaystyle \int_{0}^{1} \dfrac{x^n}{1+x^2} dx < \int_{1}^{0} x^n dx $ 성립.
    \item $1 \le (\sin x + \cos x)^2 \le 2$ 이므고 $[0,1]$에서 $ x^2 < x$ 이므로\\
    $ \int_{0}^{1} x^2 dx < \displaystyle \int_{0}^{1} x^{( \sin x + \cos x )^2} dx < \int_{0}^{1} x dx $ 성립. 
\end{enumerate}
...
\textbf{모범답안} \\
최대·최소 정리에 의해 $m \le f(x) \le M$ 이므로 $m(b-a) \le \int_a^b f(x) dx \le M(b-a)$ 가 성립하며,\\
$m \le \frac{1}{b-a} \int_a^b f(x) dx \le M$ 이고 $f$ 는 연속이므로 사잇값의 정리에 의해 만족하는 $c$ 가 존재한다. 
