...
\begin{studybox}
함수 $f(ax+b)$ (괄호 안이 일차식) 꼴의 부정적분이다.\\
이와 같은 꼴의 부정적분은 \[\int f(x)dx=F(x)+C \Rightarrow \int f(ax+b)dx=\dfrac{1}{a}F(ax+b)+C(a\ne0)\] 를 기억해 두고서 활용하는 것이 편리하다.
\end{studybox}
...
\begin{studybox}
(1)은 분자의 $x$ 를 $(x+2)-2$ 로 변형하고, \\
(2)는 분모를 유리화한다. 그리고 이들을 $\int(ax+b)^{r}dx$ 의 꼴로 변형한 다음\\ $\dfrac{1}{a}\times\dfrac{1}{r+1}(ax+b)^{r+1}+C$ 를 이용한다.
\end{studybox}
...
\begin{studybox}
$g(x)=t$ 라고 하면 $\int f(g(x))g^{\prime}(x)dx=\int f(t)dt$
\end{studybox}
...
\begin{studybox}
$\int\dfrac{f^{\prime}(x)}{f(x)}dx=\ln|f(x)|+C$
\end{studybox}
...
\begin{studybox}
$\sqrt{f(x)}$ 를 포함한 부정적분은 $\sqrt{f(x)}=t$ 또는 $f(x)=t$ 로 치환하여라.
\end{studybox}

\textbf{모범답안}
\begin{enumerate}[label=(\arabic*)]
    \item $\sqrt{1+2x}=t \Rightarrow dx=tdt$.\\
     $\int (2+3\dfrac{t^2-1}{2})t^2 dt = \dfrac{1}{15}(9x+7)(2x+1)\sqrt{1+2x}+C$
    \item $\sqrt{x+1}=t \Rightarrow dx=2tdt$. \\
    $\int 2{t^2-2} dt = 2(t^2-1)^3dt = \dfrac{2}{3}(x-5)\sqrt{x+1}+C$
    \item $\sqrt{1-x^{2}}=t \Rightarrow xdx=-tdt$.\\
     $\int -(1-t^2)^\frac{3}{2} dt = -\dfrac{1}{3}(x^{2}+2)\sqrt{1-x^{2}}+C$
\end{enumerate}
...
\begin{studybox}
$\sqrt{a^{2}-x^{2}}$ 가 있으면 $x=a\sin\theta$ 로, $\sqrt{a^{2}+x^{2}}$ 가 있으면 $x=a\tan\theta$ 로 치환한다.
\end{studybox}

\textbf{모범답안}
\begin{enumerate}[label=(\arabic*)]
    \item $\int \dfrac{1}{a^3 \cos^3\theta}a \cos\theta d\theta = \dfrac{1}{a^{2}}\tan\theta+C = \dfrac{x}{a^{2}\sqrt{a^{2}-x^{2}}}+C$
...
\textbf{모범답안}
\begin{enumerate}[label=(\arabic*)]
    \item $ v=x, u^{\prime}=e^{-x} \Rightarrow -(x+1)e^{-x}+C$
    \item $ v=x, u^{\prime}=\dfrac{1}{2}(1+\cos 2x) \Rightarrow \dfrac{1}{8}(2x^{2}+2x\sin 2x+\cos 2x)+C$
    \item $ v=\ln(x+2), u^{\prime}=1 \Rightarrow (x+2)\ln(x+2)-x+C$
\end{enumerate}
...
\textbf{모범답안}
\begin{enumerate}[label=(\arabic*)]
    \item $x^2 e^x-(2xe^x-\int 2e^x dx)=(x^{2}-2x+2)e^{x}+C$
    \item $x^2 \cos x -(-2x \sin x + \int 2 \sin x dx)=(2-x^{2})\cos x+2x\sin x+C$
    \item $x(\ln x)^2- \int 2 \ln x dx=x(\ln x)^{2}-2x\ln x+2x+C$
\end{enumerate}
...
\textbf{모범답안}
\begin{enumerate}[label=(\arabic*)]
    \item $I=\int e^{x}\cos x~dx \\
    = \cos x e^x-(-\sin x e^x + \int \cos x e^x dx) \Rightarrow I=\dfrac{1}{2}e^{x}(\sin x+\cos x)+C$
