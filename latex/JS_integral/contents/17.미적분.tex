...
\begin{wrapfigure}[2]{l}{0.18\textwidth}
    \vspace{-50pt}
    \centering
...
곡선 $y=(\ln x)^2$에 대하여 다음 물음에 답하여라 
\begin{enumerate}[label=(\arabic*)]
    \item  원점에서 이 곡선에 그은 접선의 방정식을 구하여라
    \item 이 곡선의 증감과 오목 볼록을 조사하고, 곡선의 개형을 그려라 
    \item (1)에서 구한 두 접선과 곡선의 $x \lefteqn{1}$ 인 부분으로 둘러싸인 도형의 넓이 $S$ 를 구하여라
\end{enumerate}
...
\textbf{모범답안} \\
교점을 $((\alpha, \sin 2\alpha))$라고 하자 \\
 $\sin 2x = k \cos x \implies 2 \sin x \cos x = k \cos x \implies \sin \alpha = k/2$. \\
$\int_{\alpha}^{\pi/2} (\sin 2x - k \cos x) dx = \dfrac{1}{2} \int_0^{\pi/2} \sin 2x dx = 1/2$. \\
$[-\dfrac{1}{2} \cos 2x - k \sin x]_{\alpha}^{\pi/2} = 1/2$. 두 식을 연립하여 정리하면 $k^2-4k+2=0$. $0<k<2$ 이므로 $\mathbf{k=2-\sqrt{2}}$.
...
\begin{studybox}
준 방정식에서 $y = -x \pm \sqrt{1-x^2}$ ($-1 \le x \le 1$). 직선 $y=-x$ 와 반원 $y = \pm \sqrt{1-x^2}$ 을 합친 것으로 생각한다.
\end{studybox}
