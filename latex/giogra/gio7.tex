...
\usepackage{multirow} 
\usepackage{array}
\usepackage{tcolorbox}% For proofs
...
\begin{figure}[h]
\centering
\includegraphics[width=0.4\textwidth]{sphere_center_radius.png}
\caption{구면: 중심 $C(a,b,c)$와 반지름 $r$}
\end{figure}
...
% ---- 그림 삽입 위치 2 (★가장 중요★): 원뿔 + 평면 각도별 단면 ----
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{cone_conic_sections.png}
\caption{직원추면을 자르는 평면의 각도 $\phi$에 따른 단면: 타원 / 포물선 / 쌍곡선}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.4\textwidth]{revolution_surface.png}
\caption{곡선 $f(x,y)=0$을 $y$축으로 회전시켜 만든 회전면}
\end{figure}
